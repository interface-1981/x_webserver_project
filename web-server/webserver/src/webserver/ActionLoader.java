...
public class ActionLoader {
	...
	/*
	// 引数よりActionクラスのインスタンスを生成
	public static Action load(String key) throws Exception {


		Class clazz = ClassLoader.getSystemClassLoader().loadClass(key);
		//インスタンスを生成する
		Object obj = clazz.newInstance();
		return (Action)obj;
	...
	*/
	private static ActionDefinition getActionDefinition (String key) {
		...
		loadProperties();
		if (!actionLoader.resourceBundle.containsKey(key)) {
			//Keyが見つからない場合は対象無しとする
			return null;
		}
